    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Card elevation={2} sx={{ p: 4, mb: 4 }}>
        <Stack direction={{ xs: "column", md: "row" }} sx={{ justifyContent: "space-between", alignItems: { md: "flex-end" }, gap: 2 }}>
          <Box>
            ...
            <Typography variant="h5" sx={{ mt: 1, fontSize: { xs: "2rem", md: "2.5rem" } }}>
              Prestamo #{loan.id}
            </Typography>
          </Box>
        </Stack>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, lg: 4 }}>
          <Card elevation={3} sx={{ p: 3, bgcolor: "text.primary", color: "white", height: "100%" }}>
            <Typography variant="overline" sx={{ color: "rgba(255,255,255,0.6)" }}>Parametros</Typography>
            <Stack spacing={2} sx={{ mt: 2 }}>
              {[
                ...
                <Box key={label}>
                  ...
                  <Typography variant="body2" sx={{ color: "white", fontWeight: 500 }}>{value}</Typography>
                </Box>
              ))}
            </Stack>
          </Card>
